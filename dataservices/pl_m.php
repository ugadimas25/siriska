...
try {
	...
	$stmt = $dbcon->prepare("SELECT gid, objectid_1, nama_di, desa, kecamatan, m_sept, m_okt, m_nov, m_des, m_jan, m_feb, m_mar, m_apr, m_mei, m_jun, m_jul, m_ags, public.ST_AsGeoJSON(public.ST_Transform((geom),4326),6) AS geojson FROM pl_m");
	if($stmt->execute()){
		$id_count = 0;
