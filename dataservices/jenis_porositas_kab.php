...
try {
	...
	$stmt = $dbcon->prepare("SELECT provinsi, kabupaten, kecamatan, desa, jns_tanah, porositas, public.ST_AsGeoJSON(public.ST_Transform((geom),4326),6) AS geojson FROM jenis_porositas_kab");
	if($stmt->execute()){
		$id_count = 0;
