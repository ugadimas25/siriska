...
try {
	...
	$stmt = $dbcon->prepare("SELECT gid, objectid_1, nama_di, desa, kecamatan, thp_sept, thp_okt, thp_nov, thp_des, thp_jan, thp_feb, thp_mar, thp_apr, thp_mei, thp_jun, thp_jul, thp_ags, public.ST_AsGeoJSON(public.ST_Transform((geom),4326),6) AS geojson FROM pl_thp");
	if($stmt->execute()){
		$id_count = 0;
