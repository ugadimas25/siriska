...
try {
	...
	$stmt = $dbcon->prepare("SELECT gid, objectid_1, nama_di, desa, kecamatan, k_sept, k_okt, k_nov, k_des, k_jan, k_feb, k_mar, k_apr, k_mei, k_jun, k_jul, k_ags, public.ST_AsGeoJSON(public.ST_Transform((geom),4326),6) AS geojson FROM pl_k");
	if($stmt->execute()){
		$id_count = 0;
