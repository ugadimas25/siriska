...
try {
	...
	$stmt = $dbcon->prepare("SELECT gid, objectid, ident, lat, long, altitude, k_saluran, urut, nomenklatu, nama, norec_hulu, norec_hili, kondisi, fungsi, nama_di, desa, kecamatan, public.ST_AsGeoJSON(public.ST_Transform((geom),4326),6) AS geojson FROM irigasi_saluran");
	if($stmt->execute()){
		$id_count = 0;
