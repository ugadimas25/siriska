...
try {
	...
	$stmt = $dbcon->prepare("SELECT gid, objectid_1, nama_di, desa, kecamatan, wir_sept, wir_okt, wir_nov, wir_des, wir_jan, wir_feb, wir_mar, wir_apr, wir_mei, wir_jun, wir_jul, wir_ags, public.ST_AsGeoJSON(public.ST_Transform((geom),4326),6) AS geojson FROM mt_wir");
	if($stmt->execute()){
		$id_count = 0;
