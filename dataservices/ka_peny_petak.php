...
try {
	...
	$stmt = $dbcon->prepare("SELECT gid, nama_di, desa, kecamatan, luas, pemilik, okt, nov, des, jan, feb, mar, apr, mei, jun, jul, ags, sep, public.ST_AsGeoJSON(public.ST_Transform((geom),4326),6) AS geojson FROM ka_peny_petak");
	if($stmt->execute()){
		$id_count = 0;
