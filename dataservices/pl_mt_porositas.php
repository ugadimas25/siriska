...
try {
	...
	$stmt = $dbcon->prepare("SELECT gid, objectid_1, nama_di, desa, kecamatan, por_sept, por_okt, por_nov, por_des, por_jan, por_feb, por_mar, por_apr, por_mei, por_jun, por_jul, por_ags, public.ST_AsGeoJSON(public.ST_Transform((geom),4326),6) AS geojson FROM pl_mt_porositas");
	if($stmt->execute()){
		$id_count = 0;
