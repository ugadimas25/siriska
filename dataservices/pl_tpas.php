...
try {
	...
	$stmt = $dbcon->prepare("SELECT gid, objectid_1, nama_di, desa, kecamatan, tpas_sept, tpas_okt, tpas_nov, tpas_des, tpas_jan, tpas_feb, tpas_mar, tpas_apr, tpas_mei, tpas_jun, tpas_jul, tpas_ags, public.ST_AsGeoJSON(public.ST_Transform((geom),4326),6) AS geojson FROM pl_tpas");
	if($stmt->execute()){
		$id_count = 0;
