...
try {
	...
	$stmt = $dbcon->prepare("SELECT gid, objectid, nama_di, desa, kecamatan, luas, pemilik, public.ST_AsGeoJSON(public.ST_Transform((geom),4326),6) AS geojson FROM irigasi_petak");
	if($stmt->execute()){
		$id_count = 0;
